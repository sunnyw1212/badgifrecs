...
class MyPosts extends Component {
  ...
  renderMyRecipePosts() {
    ...
    //if loading..
    if (
      !this.props.originalMyRecipePosts ||
      !this.props.originalMyRecipePosts.posts ||
      this.props.originalMyRecipePosts.loading
    ) {
        ...
        return this.props.originalMyRecipePosts.posts.map((post, index) => {
          return (
            <ListItem
              ...
              primaryText={
                <Link to={`/view/${post._id}`}> {post.recipe_title} </Link>
              }
              secondaryText={`Posted ${new Date(
                post.timestamp
              ).toLocaleDateString()}`}
              leftIcon={
                <Link
                  key={post._id}
                  to={`/view/${post._id}`}
                  className="postlist__thumbnailcontainer"
                >
                  {' '}
                  <img
            ...
            >
              <div className="postlist__btncontainer">
                ...
                <IconButton
                  tooltip="Delete"
                  onTouchTap={this.handleDeleteRecipePost.bind(this, post._id)}
                  touch={true}
                  iconStyle={{
                    color: 'rgb(117,117,117)'
                  }}
