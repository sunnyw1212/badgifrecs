...
class EditRecipe extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    if (!nextProps.originalSingle.loading && nextProps.originalSingle.post) {
      ...
      if (
        ...
        this.props.currentUser.user.name !== nextProps.currentUser.user.name
      ) {
        console.log(
          'YOU THINK YOURE SMARTER THAN ME HUH? YOUVE ACTIVATED MY REDIRECT TRAP CARD'
        );
        browserHistory.push('/');
      }
      ...
      newState['recipe_description'] =
        nextProps.originalSingle.post.recipe_description;
      newState['recipe_instructions'] = this.makeStringFromArray(
        nextProps.originalSingle.post.recipe_instructions
      );
  ...
  handleSubmit = e => {
    ...
    let currentUrl = window.location.href;
    let recipePostId = currentUrl.split('/').pop();
    this.props.actions.editRecipePost(recipePostId, {
        ...
        <form onSubmit={this.handleSubmit} encType="multipart/form-data">
          <div className="row">
            <div className="col-sm-6 col-sm-offset-3 --padlr0">
              <Card>
                <Card>
                  <CardHeader title={this.state.recipe_gif} />
                  <CardMedia className="uploadform__imgcontainer">
                    <img
                      src={this.state.recipe_gif}
                      alt="Recipe Gif"
                      className="--height100"
                    />
                  </CardMedia>
