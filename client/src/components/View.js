...
class View extends Component {
  ...
  handleNextRecipe = () => {
    ...
    //get next recipe id
    let nextRecipeId = posts[currentPostIndex + 1]
      ? posts[currentPostIndex + 1]._id
      : posts[0]._id
  ...
  handleSubmit = (e) => {
    ...
    // this.props.actions.registerUser({ 	name: this.state.name, 	password:
    // this.state.password });
    this
      .props
      .actions
      .createComment({user: this.props.currentUser.user, post_id: originalPostId, text_body: text_body});
  ...
  if (instructions.length <= 1) {
    ...
    return instructions.map((instruction, index, array) => {
      ...
      return (
        ...
        <div key={index}>
          <ListItem primaryText={instruction}></ListItem>
          <Divider></Divider>
  ...
  if (this.props.newComments.error) {
    ...
    return (
      <Dialog
        title='Oops!'
        open={true}
        actions={< RaisedButton label = 'Try Again' primary = {
        true
      }
      onTouchTap = {
        () => {
      ...
      return (
        ...
        <div key={index}>
          <ListItem
            ...
            secondaryText={< div > <p>{comment.user.name}</p> < p > {
            `${new Date(comment.timestamp).toLocaleTimeString()} ${new Date(comment.timestamp).toLocaleDateString()}`
          } < /p> </div >}></ListItem>
          <Divider></Divider>
  ...
  if (!this.props.originalAll || !this.props.originalAll.posts || this.props.originalAll.loading || this.props.originalAll.error) {
    ...
    return (
      <RaisedButton
        ...
        primary={true}
        onClick={this.handleNextRecipe}></RaisedButton>
    )
  }
}
...
render() {
  ...
  if (this.props.originalSingle.error) {
    ...
    return (
      ...
      <div className='row post__container' style={{
      height: '100%'
    }}>
      <StickyContainer>
        <div
          className='col-sm-6 col-sm-offset-1 --padlr0'
          style={{
          height: '100%'
        }}>
          <Card style={{
          ...
          <Card>
            <CardHeader
              ...
              children={this.renderNextRecipeBtn()}></CardHeader>
        ...
        <div className='col-sm-4 --padlr0'>
          <div className='viewtext__container'>
            <Card>
              <CardTitle title='Description' showExpandableButton={true} actAsExpander={true}></CardTitle>
            ...
            <Card>
              <CardTitle title='Comments'></CardTitle>
...
return {currentUser: state.users.currentUser, originalSingle: state.recipeposts.originalSingle, originalAll: state.recipeposts.originalAll, newComments: state.comment.newComments}
}
