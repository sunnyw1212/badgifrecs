...
class View extends Component {
  ...
  renderDescription() {
    ...
    if (recipe_description.length < 1) {
      return (
        <div className="--unavailable">No Description Available</div> //end if
      );
    } else {
      return <div className="--unavailable">{recipe_description}</div>;
    }
  }
  ...
  renderInstructions() {
    ...
    if (instructions.length <= 1) {
      return <div className="--unavailable">No Instructions Available</div>;
    } else {
      return instructions.map((instruction, index, array) => {
        console.log('heres the instruciton', instruction);
        return (
          <div key={index}>
            <ListItem primaryText={instruction} />
            <Divider />
          </div>
        );
      }); //end map
  ...
  render() {
    ...
    if (this.props.originalSingle.error) {
      ...
      !this.props.originalSingle ||
      !this.props.originalSingle ||
      this.props.originalSingle.loading
    ) {
