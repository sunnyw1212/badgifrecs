    ...
    if( this.props.originalAll.error){
    	...
    	console.log('errors!', this.props.originalAll)
			return(
				...
				>
					<p>{this.props.originalAll.error}</p > </Dialog>
    )
  }
        ...
        return (
          <Link key={post._id} to={`/view/${post._id}`}>
            <ListItem
              ...
              leftIcon={< img className = 'postlist__thumbnail' src = {
              post.recipe_thumb
            }
            alt = {
              post.recipe_title
...
renderRedditPosts() {
  ...
  if (!this.props.redditAll || this.props.redditAll === undefined || !this.props.redditAll.posts || this.props.redditAll.loading) {
    ...
    let gifs = posts.filter((post, index, array) => {
      if (post.data.preview && post.data.preview.images[0] && post.data.preview.images[0].variants && post.data.preview.images[0].variants.gif && post.data.permalink !== '/r/ShittyGifRecipes/comments/5z096w/meta_rgifrecipes_has_two_mobile_apps_its_abo' +
          'ut/') {
        return post
      }
      return null;
    ...
    <div>
      <List className='row'>
        <div className="col-md-6">
          <Subheader>
            BadGifRecipes Originals</Subheader>
        ...
        <div
          className='hidden-xl hidden-lg hidden-md hidden-sm floatactbtn__container--fixed'>
          <Link to='/upload'>
...
return {currentUser: state.users.currentUser, originalAll: state.recipeposts.originalAll, redditAll: state.recipeposts.redditAll}
