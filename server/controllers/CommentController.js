	...
	module.exports = {
		...
		get: function (params, callback) {
			...
			if (params.id) {
				...
				Models.CommentModel.findById(params.id, function (err, doc) {
					...
					if (err) {
						callback({
							'success': false,
							'message': 'Failed to find a Comment with the ' + params.id + ' ID'
						}, null);
						return;
					}
			...
			Models.CommentModel.find({}, function (err, docs) {
				...
				if (err) {
					callback({
						'success': false,
						'message': 'Failed to find a Comment'
					}, null);
					return;
				}
		...
		createComment: function (req, res) {
			...
			Models.CommentModel.create(comment, function (err, result) {
				...
				Models.RecipePostModel.update(recipePostQuery, {
					$push: {
						comment_ids: result._id
					}
				}, {
